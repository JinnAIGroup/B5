"""   YPL, JLL, 2021.3.22 - 2022.2.22
bz2toh5.py generates training data from rlog.bz2 for train_model5.py
...
def mkpath(all_dirs):
  for dir in all_dirs:
    ...
    if not os.path.isfile(dir + 'pathdata.h5'):
      with h5py.File(dir + 'pathdata.h5', 'w') as f:
        ...
        for ori in range(num_f-50):
          ...
          coeff = np.polyfit(x_f, y_f, 3)
          cubic_fn = np.poly1d(coeff)
          vaild_len = x_f[-1]
          x_p = np.array([1+i for i in range(50)])
          y_p = cubic_fn(x_p)
          path = np.hstack([y_p, vaild_len])
          f['Path'][ori] = path
    ...
    Path = fh5['Path']
    Velo = fh5['V']
      #---  Path.shape, Velo.shape = (1149, 51) (1149, 1)
